...
Only this module imports algo_core (to introspect the settings dataclasses);
the import is lazy so the rest of the package stays independent.
"""
...
def _load_settings_classes():
    """Lazy import of the settings dataclasses (single source of truth)."""
    ...
    from algo_core.settings import EngineParams, IndicatorParams, RiskParams
    return {"engine": EngineParams(), "risk": RiskParams(),
            "indicators": IndicatorParams()}


def build_grid(multipliers: tuple = MULTIPLIERS) -> list:
    """One-at-a-time perturbation grid over every numeric threshold."""
    instances = _load_settings_classes()
    grid: list = []
